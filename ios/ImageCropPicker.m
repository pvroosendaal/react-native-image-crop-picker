...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
	...
	if ([info[UIImagePickerControllerMediaType] isEqual:(NSString *)kUTTypeMovie]) {
		[self processVideoPick:info withViewController:picker];
		return;
	}
...
- (void)processVideoPick:(NSDictionary *)info withViewController:(UIViewController *)viewController {
	...
	[viewController dismissViewControllerAnimated:YES completion:[self waitAnimationEnd:^{
		self.resolve([self createAttachmentResponse:filePath.absoluteString
									  withSourceURL:nil
								withLocalIdentifier:nil
									   withFilename:filePath.lastPathComponent
										 ...
										 withHeight:@0
										   withMime:@"video/mp4"
										   withSize:[NSNumber numberWithUnsignedInteger:0]
...
- (void) getVideoAsset:(PHAsset*)forAsset completion:(void (^)(NSDictionary* image))completion {
	...
	NSString *timestamp = [format stringFromDate:forAsset.creationDate];
	
	NSDictionary *metadata = @{
							   @"timestamp" : timestamp,
							   @"latitude" : @(forAsset.location.coordinate.latitude),
							   @"longitude" : @(forAsset.location.coordinate.longitude)
							   };
...
- (NSDictionary*) createAttachmentResponse:(NSString*)filePath withSourceURL:(NSString*)sourceURL withLocalIdentifier:(NSString*)localIdentifier withFilename:(NSString*)filename withWidth:(NSNumber*)width withHeight:(NSNumber*)height withMime:(NSString*)mime withSize:(NSNumber*)size withData:(NSString*)data withMetadata:(NSDictionary *)metadata {
	...
	return @{
			 ...
			 @"size": size,
			 @"data": data,
			 @"timestamp": timestamp != nil ? timestamp : [NSNull null],
			 @"latitude": latitude != nil ? latitude : [NSNull null],
			 @"longitude": longitude != nil ? longitude : [NSNull null],
			 };
}
				...
				if (phAsset.mediaType == PHAssetMediaTypeVideo) {
					...
					[manager
					 ...
					 resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
						 ...
						 NSDateFormatter *format = [[NSDateFormatter alloc] init];
						 [format setDateFormat:@"yyyy-MM-dd'T'HH:mm:ss'Z'"];
						 NSString *timestamp = [format stringFromDate:phAsset.creationDate];
				...
				[manager
				 ...
				 resultHandler:^(NSData *imageData, NSString *dataUTI,
					 ...
					 dispatch_async(dispatch_get_main_queue(), ^{
						 [indicatorView stopAnimating];
						 [overlayView removeFromSuperview];
						 [self processSingleImagePick:[UIImage imageWithData:imageData] withViewController:imagePickerController withSourceURL:[sourceURL absoluteString] withLocalIdentifier:phAsset.localIdentifier withFilename:[phAsset valueForKey:@"filename"] withMetadata:metadata];
					 });
				 }];
			}
		}];
...
// or to user directly
- (void) processSingleImagePick:(UIImage*)image withViewController:(UIViewController*)viewController withSourceURL:(NSString*)sourceURL withLocalIdentifier:(NSString*)localIdentifier withFilename:(NSString*)filename withMetadata:(NSDictionary *)metadata {
